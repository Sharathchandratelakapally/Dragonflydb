...
void JournalStreamer::AsyncWrite() {
  ...
  std::vector<iovec> v(v_size);  // consider to use inline_vector and make part of JournalStreamer
  ...
  dest_->AsyncWrite(v.data(), v.size(),
                    [buf0 = std::move(pending_buf_), v0 = std::move(v), this,
                     len = pending_buf_mem_size](std::error_code ec) { OnCompletion(ec, len); });

  // AsyncWrite() sholdn't be preemtive so we can reset fields after it
